...
\begin{itemize}
    ...
    \item \ttt{INSTR\_*} correspond to the individual signals of \ttt{instr\_bus}. As the instruction bus is intended to be a read-only bus, only the corresponding signals are available. Write-related signals are tied to constant 0.
    \item \ttt{DATA\_*} correspond to the individual signals of \ttt{data\_bus}.
    \item \ttt{IRQ*} correspond to the individual signals of \ttt{int\_bus}.
    \item Other signals from the SoC interface are not available in this basic interface (\ttt{control\_flags}, \ttt{core\_int\_triggers}, \ttt{gpio\_*} and \ttt{pwm} aside from exceptions below). If they are needed, please use the generic \ttt{edusoc} module instead.
\end{itemize}
\newpage
